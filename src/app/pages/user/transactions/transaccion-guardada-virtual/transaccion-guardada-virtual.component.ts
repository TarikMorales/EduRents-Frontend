...
export class TransaccionGuardadaVirtualComponent implements OnInit {
      ...
      this.transactionService.getTransactionByIdAndUser(this.idTransaccion, this.idUsuario, this.token).subscribe({
        next: (transaction) => {
          if (!transaction) {
            ...
            this.idProducto = transaction.producto.id;
            ...
            if (transaction.estado === 'PAGADO' || transaction.estado === 'CANCELADO') {
  ...
  confirmarPago(): void {
    this.transactionService.confirmarTransaccion(this.idTransaccion, this.token).subscribe({
      next: () => {
        this.transactionService.actualizarStockProducto(this.idProducto, this.token).subscribe({
          next: () => this.showSnackBar('Pago confirmado y stock actualizado'),
          error: () => this.showSnackBar('Error al actualizar el stock del producto')
        });
